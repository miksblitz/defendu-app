// app/(tabs)/topup-invoice.tsx
// Receipt shown after a successful credit top-up.
...
export default function TopUpInvoicePage() {
  ...
  const phpAmount = Number(params.phpAmount ?? 0);
  const balanceAfter = Number(params.balanceAfter ?? 0);
  const createdAt = Number(params.createdAt ?? Date.now());
  const paymentMethod = (params.paymentMethod ?? 'card').toUpperCase();
  const invoiceId = params.transactionId ?? '—';

  const rows: { label: string; value: string; highlight?: boolean }[] = [
    { label: 'Invoice No.', value: invoiceId.slice(-10).toUpperCase() },
    { label: 'Date', value: formatDate(createdAt) },
    { label: 'Payment Method', value: paymentMethod },
    { label: 'Amount Paid', value: `₱${phpAmount.toFixed(2)}` },
    { label: 'Credits Added', value: `+${credits.toLocaleString()} cr`, highlight: true },
    { label: 'New Balance', value: `${balanceAfter.toLocaleString()} cr` },
  ];
  ...
  return (
    <SafeAreaView style={styles.root}>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <Text style={styles.subheading}>Credits have been added to your wallet.</Text>
