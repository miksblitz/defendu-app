// POST /api/pose-estimation-ticket
// Sends a developer ticket via Mailjet to request pose estimation work for a module.
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!mailjetApiKey || !mailjetApiSecret) {
      console.error('Mailjet credentials missing for pose-estimation-ticket');
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 720px; margin: 0 auto; padding: 20px;">
  ...
  <div style="background-color: #f9f9f9; padding: 28px; border-radius: 0 0 10px 10px; border: 1px solid #e0e0e0; border-top: none;">
    ...
    console.error('pose-estimation-ticket error:', message);
